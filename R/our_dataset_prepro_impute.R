...
# # Impute using the phased genotypes
# # Option 1: shapeit without 1000G as ref
# shapeit_out_path = paste(job_dir,"shapeit_out/",sep="")
# impute2_out_path = paste(job_dir,"impute2_out/",sep="")
...
#     chunk2out[chunk_count] = curr_out
...
for(j in 1:22){
  ...
  # Clean the directory and gzip the imputed file
  run_archive_dir = paste("chr",j,"_run_archive",sep="")
  system(paste("mkdir",run_archive_dir))
  for(ff in list.files(getwd())){
    if(grepl("chunck",ff)){system(paste("mv",ff,run_archive_dir))}
  }
